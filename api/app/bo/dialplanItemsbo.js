...
class dialplanItemsbo extends baseModelbo {
    ...
    getDialPlan (req, res, next){
        ...
        this.db['users'].findOne({
            ...
            let agent_dataJson = agent_data.toJSON()
            if (agent_dataJson.config && agent_dataJson.config.cli === 'CLI FIX') {
                let sql = `select *
                           from truncks tr
                           where tr.trunck_id = (select dialplan.trunck_id
                                                 from dialplan_items dialplan
                                                 where dialplan.active = 'Y'
                                                   and dialplan.status = 'Y'
                                                   and :phone like concat(dialplan.prefix, '%')
                                                   and :cli like concat(dialplan.pai, '%')
                                                   and dialplan.account_id = :account_id)
                ...
                db.sequelize['crm-app'].query(sql, {
                    ...
                    replacements: {
                        ...
                        cli: agent_dataJson.config.value
                    }
                }).then(result => {
                ...
                db.sequelize['crm-app'].query(sql, {
                    ...
                    if (dialplan && dialplan.length !== 0) {
                        ...
                        dialplan.forEach(item => {
                            let sqlCli = `select did.number
                                          from dids did
                                          where did.did_group_id in (:did_group)
                                            and did.active = 'Y'
                                            and did.status = 'Y'
                                            and did.number like :pai
                                          ORDER BY RANDOM()
                            `
                            ...
                            db.sequelize['crm-app'].query(sqlCli, {
                                type: db.sequelize['crm-app'].QueryTypes.SELECT,
                                replacements: {
                                    did_group: agent_dataJson.config.did_group_ids,
                                    pai: item.pai + '%'
                                }
                            }).then(result => {
                                if (result && result.length !== 0) {
